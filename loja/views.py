...
from .seriliazers.srzBuypecas import BuyPecasSerializer
...
class BuyPecasViewSet(viewsets.ModelViewSet):
    queryset = BuyPecas.objects.all()
    serializer_class = BuyPecasSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = StandardResultsSetPagination

    @transaction.atomic
    def perform_create(self, serializer):
        part_buy = serializer.save()
        peca = part_buy.peca
        quantity_usage = part_buy.quantidade

        if peca.quantidade_em_estoque < quantity_usage:
            raise serializers.ValidationError("Quantidade em estoque insuficiente para esta peça.")

        peca.quantidade_em_estoque -= quantity_usage
        peca.save()

        MovimentacaoEstoque.objects.create(
            peca=peca,
            tipo_movimentacao='SAIDA',
            quantidade=quantity_usage,
            usuario_responsavel=self.request.user,
            origem_destino=f"Aquisição de Peça {part_buy.id}",
        )
    @transaction.atomic
    def perform_destroy(self, instance):
        part = instance.part

        if instance.tipo_movimentacao == 'SAIDA':
            part.stock_quantity += instance.quantidade
